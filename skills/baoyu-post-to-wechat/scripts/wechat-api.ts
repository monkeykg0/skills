...
async function publishToDraft(
  ...
  const url = `${updateMediaId ? DRAFT_UPDATE_URL : DRAFT_URL}?access_token=${accessToken}`;
  ...
  const res = await client(url, {
    ...
    body: JSON.stringify(updateMediaId
      ? { media_id: updateMediaId, index: 0, articles: article }
      : { articles: [article] }),
  });
  ...
  return updateMediaId ? { ...data, media_id: updateMediaId } : data;
}
...
Options:
  ...
  --update-media-id <id>  Update article index 0 in an existing draft instead of creating a new draft
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--type" && argv[i + 1]) {
      ...
      args.dryRun = true;
    } else if (arg === "--update-media-id" && argv[i + 1]) {
      args.updateMediaId = argv[++i];
    } else if (arg === "--remote") {
      args.remote = true;
...
async function main(): Promise<void> {
  ...
  const publishWith = async (client: WechatClient): Promise<void> => {
    ...
    console.error(args.updateMediaId
      ? `[wechat-api] Updating existing draft: ${args.updateMediaId}`
    ...
    }, accessToken, client, args.updateMediaId);
    ...
    console.log(JSON.stringify({
      ...
      articleType: args.articleType,
      method,
      updated: Boolean(args.updateMediaId),
    }, null, 2));
